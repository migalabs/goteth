...
// 	for _, item := range p.baseMetrics.ThirdState.PrevAttestations {
...
// 			p.baseMetrics.ThirdState.AttestingVals[attestingValIdx] = true

// 			// add correct flags and balances
// 			if p.IsCorrectSource() && p.baseMetrics.ThirdState.CorrectFlags[altair.TimelySourceFlagIndex][attestingValIdx] == 0 {
// 				p.baseMetrics.ThirdState.CorrectFlags[altair.TimelySourceFlagIndex][attestingValIdx] += 1
// 				p.baseMetrics.ThirdState.AttestingBalance[altair.TimelySourceFlagIndex] += p.baseMetrics.ThirdState.Validators[attestingValIdx].EffectiveBalance
// 			}

// 			if p.IsCorrectTarget(*item) && p.baseMetrics.ThirdState.CorrectFlags[altair.TimelyTargetFlagIndex][attestingValIdx] == 0 {
// 				p.baseMetrics.ThirdState.CorrectFlags[altair.TimelyTargetFlagIndex][attestingValIdx] += 1
// 				p.baseMetrics.ThirdState.AttestingBalance[altair.TimelyTargetFlagIndex] += p.baseMetrics.ThirdState.Validators[attestingValIdx].EffectiveBalance
// 			}

// 			if p.IsCorrectHead(*item) && p.baseMetrics.ThirdState.CorrectFlags[altair.TimelyHeadFlagIndex][attestingValIdx] == 0 {
// 				p.baseMetrics.ThirdState.CorrectFlags[altair.TimelyHeadFlagIndex][attestingValIdx] += 1
// 				p.baseMetrics.ThirdState.AttestingBalance[altair.TimelyHeadFlagIndex] += p.baseMetrics.ThirdState.Validators[attestingValIdx].EffectiveBalance
// 			}

// 			// we also organize which validator attested when, and when was the attestation included
// 			if val, ok := p.baseMetrics.ThirdState.ValAttestationInclusion[attestingValIdx]; ok {
// 				// it already existed
// 				val.AddNewAtt(uint64(slot), uint64(inclusionSlot))
// 				p.baseMetrics.ThirdState.ValAttestationInclusion[attestingValIdx] = val
// 			} else {
...
// 				p.baseMetrics.ThirdState.ValAttestationInclusion[attestingValIdx] = newAtt
...
// 	duties := append(p.baseMetrics.ThirdState.EpochStructs.ProposerDuties, p.baseMetrics.PrevState.EpochStructs.ProposerDuties...)
...
// 	if isProposer {
// 		votesIncluded := 0
// 		for _, valAttestation := range p.baseMetrics.ThirdState.ValAttestationInclusion {
